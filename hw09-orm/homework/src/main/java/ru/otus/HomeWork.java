...
// Сделайте тоже самое с классом Manager (для него надо сделать свою таблицу)
        ...
        managerSecond.setLabel("updated label");
        dbServiceManager.saveManager(managerSecond);
        var managerSecondSelectedAfterUpdate = dbServiceManager.getManager(managerSecond.getNo())
                .orElseThrow(() -> new RuntimeException("Manager not found, id:" + managerSecond.getNo()));
        log.info("managerSecondSelectedAfterUpdate:{}", managerSecondSelectedAfterUpdate);

        List<Manager> all = dbServiceManager.findAll();
        log.info("all managers:{}", all);
